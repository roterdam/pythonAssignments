...
class LearningSwitch(DynamicPolicy):
	...
	self.forwardTable = []
  	self.forwardTable =[(1, "00:00:00:00:00:01", 3), ( 1, "00:00:00:00:00:02", 2)]
	...
	for rule in (self.forwardTable):
            # print "------"
	    print pkt['switch'] == int(rule[0])
	    print str(pkt['srcmac']) == rule[1]
	    print pkt['inport'] == int(rule[2])
     
            print pkt['switch'] 
	    print str(pkt['srcmac'])
            print pkt['inport']

	    #print "-----"		

 	    if pkt['switch'] == int(rule[0]) and str(pkt['srcmac']) == rule[1] and pkt['inport'] == int(rule[2]):
                print "FOUND1"
		found = True
		break
	    else: 
	...
	print found
        if found == False:
            self.forwardTable.append((pkt['switch'], str(pkt['srcmac']), pkt['inport']))
